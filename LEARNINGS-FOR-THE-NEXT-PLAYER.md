...
What a route pays depends on the destination's districts, the policies and trading posts on the path, governors, and what the origin needs; international routes also carry tourism and alliance value that a yield number doesn't show. Do not derive a routing rule from these two readings — the recorder now logs per-route yields every poll, so learn it from the games. Build capacity early (Markets, Lighthouses, Harbors, Seaports) and keep it full. Fable ended with 859 gold banked ≈ a full turn of its empire never spent; spend every turn.
