...
while true; do
  ...
  if ! tasklist.exe 2>/dev/null | grep -qi obs64.exe || [ -z "$(find frames/hls -name 'obs_*.ts' -mmin -1 2>/dev/null)" ]; then
  ...
  if [ "$n" -lt 2 ]; then echo "$(date +%T) restart transcoder"; (setsid nohup cmd.exe /c 'C:\Users\danie\cc\civilization.sh\video\stream.cmd' >/tmp/stream.log 2>&1 &); fi
