...
const planMd = await (async () => { for (const p of ["../PLAN.md"]) { try { return await Deno.readTextFile(p); } catch { /* next */ } } return "# Plan\n\nSealed until the next benchmark game is played. See BENCHMARK.md to play the board yourself."; })();
