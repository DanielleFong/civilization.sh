...
const REG_VARS = { model: ["Fable 5.1", "Opus 5", "Sonnet 5", "GPT-5.5", "GPT-6", "Astra", "Gemini 3.1 Pro", "human", "other"], agent: ["agent", "agent + human advisor", "agent civilization", "human"], game: ["Civilization VI", "Civilization V", "Civilization IV", "Alpha Centauri"], map: ["Earth TSL", "Continents", "Pangaea", "Small Continents"], mode: ["FFA", "teams 2v2", "1v1"], difficulty: ["King", "Emperor", "Immortal", "Deity", "Mythic", "Transcendent", "Infernal", "Primordial", "Sid Meier", "Prince", "Noble", "Monarch", "Citizen", "Specialist", "Talent", "Librarian", "Thinker", "Transcend"] };
...
async function authRoutes(req, env, u, p) {
  ...
  if (p === "/auth/steam/callback") { const q = new URLSearchParams(u.search); q.set("openid.mode", "check_authentication"); const ok = (await (await fetch("https://steamcommunity.com/openid/login", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: q })).text()).includes("is_valid:true");
    ...
    if (!name || name === "steam:" + steamid || /^\d+$/.test(String(name))) { try { const xml = await (await fetch(`https://steamcommunity.com/profiles/${steamid}/?xml=1`, { headers: { "user-agent": "civilization.is ladder" } })).text(); const m = xml.match(/<steamID><!\[CDATA\[([^\]]*)\]\]><\/steamID>/); if (m && m[1]) name = m[1].slice(0, 60); const a = xml.match(/<avatarMedium><!\[CDATA\[([^\]]*)\]\]><\/avatarMedium>/); if (a && !avatar) avatar = a[1]; } catch {} } // no API key: public profile XML carries the persona name
